...
Shopify.theme.jsProduct = {
  init: function init($section) {
    ...
    if ($productGallery) {
      ...
      if (Shopify.theme.jsProduct.enable_zoom) {
        document.addEventListener('lazybeforeunveil', Shopify.theme.jsProduct.enableZoom);
      }
  ...
  findSelectedVariantImage: function findSelectedVariantImage() {
    getIndex($('[data-variant-selector]'), $('[data-variant-selector]').val());

    $('[data-variant-selector]').on('selectedVariantChanged', function () {
      
      if ( !jQuery.isEmptyObject(metaData) ){
        $(".product-page .description").empty().html(metaData[$(this).val()]);
      }      
    //   console.log("variant", $(this).val());
    //   console.log("variant", $(this));
    //   setTimeout(function() { 
    //     var x = $(".current_price").html();
    //     x = x.substring(1);
        
    //     let y = x * 30 / 100;
    //     x = x - y;
    //   console.log(x);
    //     $(".sale_price").html("$" + x.toFixed(2) + "&nbsp");
    // }, 1000);
      
     // $(".sale_price").append("<span>"+ $(this)  +"</span>");
      //js code to display the shipping date when choosing scent diffuser black variant
      // if ( $(this).val() == 40369869291686 ){        
      //   $(".custom-shipping-text-scent-diffuser").removeClass("hidden");
      // }else{        
      //   if ( $(".custom-shipping-text-scent-diffuser").length ){
      //     $(".custom-shipping-text-scent-diffuser").addClass("hidden");
      //   }
      // }
    ...
    function getIndex($selector, variantID) {
      ...
      var $productGallery = $(document).find('.product-gallery__main');
      $productGallery.flickity( 'select', index );
...
// $(window).on('load', function(){
//   setTimeout(function(){
// //     $('.product-gallery__image').eq(0).addClass('is-selected');
// //     $('.product-gallery__image').eq(0).siblings().removeClass('is-selected');
// // //     $('.product-gallery__thumbnail').eq(0).addClass('is-nav-selected').siblings().removeClass('is-nav-selected');
// //      $('.product-gallery__thumbnails .product-gallery__thumbnail').eq(0).trigger('click');
//     $('.product-gallery__thumbnails .product-gallery__thumbnail').eq(0).trigger('click');
//   }, 100);
// });
...
$(document).ready(function() {
  $(document).on('click', '.rc_widget__option__selector', function(e) {
    $(document).trigger("recharge:loaded", e)
  })
})