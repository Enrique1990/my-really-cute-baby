...
var selectCallback = function selectCallback(variant, selector) {
  ...
  // update discounted price on variant change

  var salePercentage = parseFloat('25')

  var sellingPlan = $('.rc-radio__input:checked').val()
  var variantPrice = variant.price / 100
  if (sellingPlan == 'subsave') {
    variantPrice = variant.selling_plan_allocations[0].price / 100 * parseInt($('.quantity-input').val())
  }
  
  var newPrice = (roundNumberV2(roundNumberV2((variantPrice * ((100 - salePercentage) / 100)), 3), 2) * parseInt($('.quantity-input').val())).toFixed(2)
  $(".sale_price").html("$" + newPrice + " &nbsp");
  
  $(document).on('recharge:loaded', function(e, elem) {
    var sellingPlan = $(elem.target).val()
    var variantPrice = variant.price / 100
    if (sellingPlan == 'subsave') {
      variantPrice = variant.selling_plan_allocations[0].price / 100 * parseInt($('.quantity-input').val())    
    }
  ...
  if (variant && variant.available == true) {
    if (variant.price < variant.compare_at_price) {
      ...
      var percentage_off = ( variant.compare_at_price - variant.price ) * 100 / variant.compare_at_price;
      $('.was-price-percentage .save-amount', $product).html('<span class="money-off">' + percentage_off + "% off" + '</span>');
    } else {
      $('.was-price', $product).html('');
      $('.savings', $product).html('');
      $('.current_price', $product).parent().removeClass('sale');
      $('.was-price-percentage .save-amount', $product).html('');
    }

    // shipment info hidden for certain variant Scent diffuser black varaint
    //if (variant.id == 40369869291686) {
    //  $('.custom-shipping-text-scent-diffuser', $product).html("Estimated ship date: <strong>June 29th</strong>");
    //} else {
    //  $('.custom-shipping-text-scent-diffuser', $product).html(" ");
    //}
    ...
    if (variant.price > 0) {
      ...
      var saleValComputation = 100 * variantPrice * ((100 - parseInt($('[data-saleval]').attr('data-saleval')))/100.00)
  console.log(saleValComputation, 'sale val computation', !isNaN(saleValComputation), isNaN(0), variantPrice)
        if (sellingPlan == 'subsave'){
                    if (variant.compare_at_price){

                  $('[data-price]').attr('data-price', 100 * variant.selling_plan_allocations[0].price)
                  $('[data-compare-price]').attr('data-compare-price', variant.compare_at_price)
                  } else {
                    if (!isNaN(saleValComputation)){
                      $('.modal_price [data-price]').attr('data-price', 100 * variantPrice * ((100 - parseInt($('[data-saleval]').attr('data-saleval')))/100.00))
                      $('[data-compare-price]').attr('data-compare-price', 100 * variantPrice)
                    } else {
                          $('[data-price]').attr('data-price', 100 * variantPrice)
                    }

                  }
        } else {
                    if (variant.compare_at_price){
                  $('[data-price]').attr('data-price', variant.price)
                  $('[data-compare-price]').attr('data-compare-price', variant.compare_at_price)
                  } else {
                    if (!isNaN(saleValComputation)){
                      console.log('computing with saleval')
                      $('.modal_price [data-price]').attr('data-price', 100 * variantPrice * ((100 - parseInt($('[data-saleval]').attr('data-saleval')))/100.00))
                      $('[data-compare-price]').attr('data-compare-price', 100 * variantPrice)
                    } else {
                      console.log('computing variant price', 100* variantPrice)
                          $('[data-price]').attr('data-price', 100 * variantPrice)
                    }
                  }
        }
    ...
    if (results.length > 0) {
      ...
      for (var _i4 = 0; _i4 < Shopify.theme_settings.search_to_display; _i4++) {
          if (results[_i4] != null) {
            ...
            if (result.object_type == 'product' && result.title != 'Scent Diffuser WIFI' && result.title != 'Scent Diffuser Starter Kit') {
            ...
            if (result.thumbnail != '' && result.title != 'Scent Diffuser WIFI' && result.title != 'Scent Diffuser Silver' && result.title != 'Scent Diffuser Starter Kit') {
            ...
            if (result.title != 'Scent Diffuser WIFI' && result.title != 'Scent Diffuser Silver' && result.title != 'Scent Diffuser Starter Kit') {
      ...
      if (results.length > Shopify.theme_settings.search_to_display && result.title != 'Scent Diffuser WIFI' && result.title != 'Scent Diffuser Silver' && result.title != 'Scent Diffuser Starter Kit' ) {
